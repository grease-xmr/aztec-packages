...
use crate::barretenberg_api::utils::Buffer;
use std::ptr;

/// AES-128 CBC encryption
/// Expects input to be already padded (PKCS#7 padding should be applied at the TypeScript layer)
pub unsafe fn aes_encrypt_buffer_cbc(input: &[u8], iv: &[u8; 16], key: &[u8; 16]) -> Buffer {
    // Verify input is properly padded (must be multiple of 16 bytes)
    if input.len() % 16 != 0 {
        panic!(
            "Input length must be a multiple of 16 bytes (pre-padded), got: {}",
            input.len()
        );
    }
...
pub unsafe fn aes_decrypt_buffer_cbc(input: &[u8], iv: &[u8; 16], key: &[u8; 16]) -> Buffer {
    // The input here should be the actual encrypted data (multiple of 16 bytes)
    if input.len() % 16 != 0 {
        panic!(
            "Input length must be a multiple of 16 bytes for AES decryption, got: {}",
            input.len()
        );
    }
    ...
    let decrypted_buffer =
        Buffer::from_ptr(result_ptr as *const u8).expect("AES decryption failed");
