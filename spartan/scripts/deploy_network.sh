...
TOTAL_PUBLISHERS=$((TOTAL_VALIDATOR_KEYS * PUBLISHERS_PER_VALIDATOR_KEY))

if (( TOTAL_PUBLISHERS > 0 )); then
  PUBLISHER_RANGE=$(seq "$VALIDATOR_PUBLISHER_MNEMONIC_START_INDEX" $((VALIDATOR_PUBLISHER_MNEMONIC_START_INDEX + TOTAL_PUBLISHERS - 1)) | tr '\n' ',' | sed 's/,$//')
  # Append publisher indices to prefund list
  LABS_INFRA_INDICES="${LABS_INFRA_INDICES},${PUBLISHER_RANGE}"
fi
